...
function createEditSession(session: any) {
    ...
    (props, forwardRef) => {
        ...
        useEffect(() => {
            ...
            if (edt) {
                ...
                if (props.session) {
                    let sessionToRestore = JSON.parse(props.session);

                    let newSession = createEditSession(sessionToRestore);
                    editor.setSession(newSession);
                }
            }

            return () => {
                if (edt && props.onSaveSession) {
                    let sessionToSave = JSON.stringify(editor.getSession());
        ...
        useEffect(() => {
            ...
            if (edt) {
                let isInAndSupported = Object.values(LoideLanguages).find(
                    (lang) => {
                        return (
                            lang.name === props.mode && lang.highlightSupport
                        );
                    }
                );
                if (isInAndSupported) {
